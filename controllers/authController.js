...
exports.verifyVerificationCode = async (req, res) => {
    ...
    try {
        ...
        if(Date.now() - existingUser.verificationCodeValidation > 5* 60 *1000){
            return res
            .status(400)
            .json({success: false, message: 'Code has been expireed'});
        }
        const hashedCodeValue = hmacProcess(codeValue, process.env.
            ...
            if(hashedCodeValue === existingUser.verificationCode){
                ...
                return res
                .status(200)
                .json({ success: true, message: 'Your account has een Verified!'})
            }
            return res
            .status(400)
...
exports.changePassword = async (req, res) => {
    ...
    try {
        const { error, value } = changePasswordSchema.validate({ oldPassword,
            newPassword });
            if(error) {
                return res
                .status(401)
...
exports.verifyForgotPasswordCode = async (req, res) => {
    ...
    try {
        const { email, value } = acceptFPCodeSchema.validate({ email, providedCode, newPassword});
        if (error) {
            return res
            .status(401)
            .json({ success: false, message: error.details[0].message});
        ...
        if(Date.now() -existingUser.forgotPasswordCodeValidation > 5 * 60 * 1000) {
            return res
            .status(400)
            .json( {success: false, message: 'Code has been exopired!'});
        }
